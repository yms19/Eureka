...
def compute_reward(dof_pos: torch.Tensor, dof_vel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Get the pole position and velocity from the observation
    pole_pos = dof_pos[..., 0]
    pole_vel = dof_vel[..., 0]

    # Calculate the pole angle using the position and velocity
    pole_angle = torch.atan2(pole_pos, pole_vel)

    # Define the target angle as 0 degrees (upright position)
    target_angle = torch.tensor(0.0, device=pole_angle.device)

    # Calculate the angular deviation of the pole from the target angle
    angle_deviation = torch.abs(pole_angle - target_angle)

    # Calculate the exponential decay reward based on the angle deviation
    reward_dist = torch.exp(-angle_deviation)

    # Total reward is based only on the distance reward in this case
    total_reward = reward_dist
